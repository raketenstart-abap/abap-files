CLASS zcl_core_frontend_service_wrap DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.

    INTERFACES zif_core_frontend_service_wrap .

    CONSTANTS:
      BEGIN OF sc_file_open_dialog .
    CONSTANTS cancel TYPE i VALUE '9'.
    CONSTANTS: END OF sc_file_open_dialog .
    CONSTANTS BEGIN OF sc_file_type .
...
CLASS ZCL_CORE_FRONTEND_SERVICE_WRAP IMPLEMENTATION.


  METHOD zif_core_frontend_service_wrap~file_open_dialog.
...
*        with_encoding           =
    ...
    IF sy-subrc <> 0.
      ...
      RAISE EXCEPTION TYPE zcx_core_frontend_service_wrap
        EXPORTING
          textid     = zcx_core_frontend_service_wrap=>open_failed
  ...
  METHOD zif_core_frontend_service_wrap~file_save_dialog.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_core_frontend_service_wrap
        EXPORTING
          textid = zcx_core_frontend_service_wrap=>save_failed.
    ENDIF.

  ENDMETHOD.


  METHOD zif_core_frontend_service_wrap~get_computer_name.
  ...
  METHOD zif_core_frontend_service_wrap~get_ip.
  ...
  METHOD zif_core_frontend_service_wrap~gui_download.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_core_frontend_service_wrap
        EXPORTING
          textid      = zcx_core_frontend_service_wrap=>download_failed
  ...
  METHOD zif_core_frontend_service_wrap~gui_upload.

    DATA lv_file_length TYPE int4.
    DATA lt_data_tab    TYPE ztt_core_file_raw_512.
...
*        isscanperformed         = SPACE
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_core_frontend_service_wrap
        EXPORTING
          textid      = zcx_core_frontend_service_wrap=>upload_failed
          mv_filename = iv_filename.
    ENDIF.

    return-filename        = iv_filename.
    return-bin_size        = lv_file_length.
    return-bin_content_tab = lt_data_tab.
